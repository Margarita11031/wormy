...
function ChangeDirection(actKey) {
    ...
    switch (actKey.keyCode) {
        case 40: newVal = "D"; break;
        case 39: newVal = "R"; break;
        case 38: newVal = "U"; break;
        case 37: newVal = "L"; break;
        default: return;
    }
    ...
    var revertDir = function(){
        for (var i = 0; i < wormPos.length ; i++) {
            switch (wormPos[i][3]) {
                case "R": wormPos[i][3] = "L"; break;
                case "L": wormPos[i][3] = "R"; break;
                case "U": wormPos[i][3] = "D"; break;
                case "D": wormPos[i][3] = "U"; break;
                default: break;
            }
        }

        for (var j = 0; j < changeDirSpot.length ; j++) {
            if (changeDirSpot[j][4] != 0 ) {changeDirSpot[j][4] = wormPos.length - changeDirSpot[j][4]};
            tempDir = changeDirSpot[j][3];
            changeDirSpot[j][3] = changeDirSpot[j][2];
            changeDirSpot[j][2] = tempDir;
            //direction to
            switch (changeDirSpot[j][2]) {
                case "R": changeDirSpot[j][2] = "L"; break;
                case "L": changeDirSpot[j][2] = "R"; break;
                case "U": changeDirSpot[j][2] = "D"; break;
                case "D": changeDirSpot[j][2] = "U"; break;
                default: break;
            }
            // direction from
            switch (changeDirSpot[j][3]) {
                case "R": changeDirSpot[j][3] = "L"; break;
                case "L": changeDirSpot[j][3] = "R"; break;
                case "U": changeDirSpot[j][3] = "D"; break;
                case "D": changeDirSpot[j][3] = "U"; break;
                default: break;
            }
        }
    }
...
function GetNewSegmPos_tbl(nbrPrevSegm, nbrRad){
    ...
    switch ((wormPos[nbrPrevSegm+1]|| wormPos[nbrPrevSegm])[3]) {
        case "R": {nbrSegmX = nbrSegmX - nbrRadBody - nbrRad + nbrSegmConnection; break;}
        case "L": {nbrSegmX = nbrSegmX + nbrRadBody + nbrRad - nbrSegmConnection; break;}
        case "U": {nbrSegmY = nbrSegmY + nbrRadBody + nbrRad - nbrSegmConnection; break;}
        case "D": {nbrSegmY = nbrSegmY - nbrRadBody - nbrRad + nbrSegmConnection; break;}
        default: break;
    }
        ...
        for (var j = 0; j < changeDirSpot.length; j++) {
            if (changeDirSpot[j][4] !=0){
                if (wormPos[segmPos][0] == changeDirSpot[j][0] && wormPos[segmPos][1] == changeDirSpot[j][1]) {
                    ...
                    changeDirSpot[j][4] -= 1;
                    if (changeDirSpot[j][4] == 0) { changeDirSpot.splice(j,1); };
                }
            }
        }
        switch (wormPos[segmPos][3]) {
            case "R": wormPos[segmPos][0] += 1; break;
            case "L": wormPos[segmPos][0] -= 1; break;
            case "U": wormPos[segmPos][1] -= 1; break;
            case "D": wormPos[segmPos][1] += 1; break;
            default: break;
        }
    }
}